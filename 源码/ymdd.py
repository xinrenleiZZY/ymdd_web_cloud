...
def convert_files(source_file):
    ...
    try:
        ...
        # 加载隐藏表格文件
        hidden_wb = load_workbook('mnt\隐藏表格.xlsx', data_only=True)

        # 保存订单录入文件
        order_filename = os.path.join(save_dir, f'结果文件/订单录入结果_{timestamp}.xlsx')
        ...
        # 保存工件导入文件
        workpiece_filename = os.path.join(save_dir, f'结果文件/工件导入结果_{timestamp}.xlsx')
